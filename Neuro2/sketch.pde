Arrow[] meteors = new Arrow[10]; //obstacles
...
void timeToDie() //here it is
{
  ...
  { //creating champion's children
    ...
    { //if the player is champion or have beaten a highscore
      ...
      players[i].neW(players[champion].nn, false);
      players[i].isChampion=true; //colour parameter
      continue;
    }
    if (num>30)
    {
      players[i].neW(players[champion].nn, true);
    } else if (num>20)
    {
      players[i].neW(players[champion2].nn, true);
    } else if (num>10)
    {
      players[i].neW(players[champion3].nn, true);
    } else
    {
      players[i].neW(players[champion4].nn, true);
    }
    num--;
    //look Player → neW()
  }
